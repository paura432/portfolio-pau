...
export const caseDetails: Record<'ovi' | 'duplex' | 'broki' | 'trustos', CaseDetail> = {
  ovi: { role: 'Full Stack Product Engineer', builtAt: 'Performanze', year: '2026 — now', scope: ['Product workflows', 'Microsoft context', 'Artifacts'], stack: ['React', 'Supabase', 'Microsoft Graph'], challenge: 'Turn work requests into reliable actions across Microsoft 365 context.', decisions: [{ decision: 'Structured product routing', why: 'Actions need explicit intent and context.', tradeOff: 'More contracts than a pure chat interface.' }, { decision: 'Conversation state stays bounded', why: 'Follow-up work needs current context without treating every interaction as permanent memory.', tradeOff: 'Useful context must be selected rather than retained indiscriminately.' }, { decision: 'Semantic context is retrieved when relevant', why: 'Embeddings can recover related knowledge by meaning, not only exact keywords.', tradeOff: 'Retrieved context remains a supporting input, not an automatic action.' }], delivered: ['AI-assisted productivity workflows', 'Microsoft 365 context integration', 'Artifact generation integration'], quality: ['Husky pre-commit gates', 'Playwright browser checks', 'Public-safe conceptual demo'], boundary: 'No tenant data, credentials, private payloads or exact architecture.' },
  duplex: { role: 'Technical leadership on SAT module', builtAt: 'Performanze', year: '2026 — now', scope: ['Technical workflows', 'Retrieval', 'Source traceability'], stack: ['Next.js', 'Supabase', 'PostgreSQL', 'RAG'], challenge: 'Keep technical diagnosis grounded in equipment context and traceable sources.', decisions: [{ decision: 'Evidence before diagnosis', why: 'Technical answers need manuals, checklists and history.', tradeOff: 'More explicit source handling than generic chat.' }, { decision: 'Equipment context is active state', why: 'The current equipment determines which technical evidence is relevant.', tradeOff: 'Context must remain explicit throughout a diagnostic session.' }, { decision: 'Trace follows the answer', why: 'Technicians need to distinguish supported guidance from a generic suggestion.', tradeOff: 'Sources and their public-safe boundaries need visible handling.' }], delivered: ['SAT assistant workflow', 'Source-backed diagnosis', 'Escalation context'], quality: ['Husky pre-commit gates', 'Playwright browser checks', 'Public-safe sanitized reconstruction'], boundary: 'No manuals, customer data, equipment records or private integrations.' },
