...
export default function PortfolioShell() {
  ...
  return <main className="live-home">
    ...
    <div id="main-content">
      ...
      <section id="work" className="live-section selected-work" aria-labelledby="work-title"><div className="section-label"><span>01</span><h2 id="work-title">PROFESSIONAL SYSTEMS</h2></div><div className="work-list">{systems.map((project) => <Link href={'/work/' + project.slug} key={project.slug} className={'work-row work-' + project.slug}><span>{project.index}</span><div><h3>{project.title}</h3><p>{project.chapter}</p></div><p>{project.company} / {project.period}</p><ArrowUpRight aria-hidden="true" /></Link>)}</div></section>
      <section className="independent-callout"><p className="kicker">02 / INDEPENDENT PRODUCT</p><h2>{independentWork.title}</h2><p>{independentWork.summary}</p><p className="mono-line">{independentWork.technologies.join(' · ')}</p><Link href="/work/broki">OPEN CASE STUDY <ArrowUpRight size={18} /></Link></section>
