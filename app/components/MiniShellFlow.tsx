...
export default function MiniShellFlow() {
  ...
  const tokens = useMemo<string[]>(() => command.match(/(?:[^\s"']+|"[^"]*"|'[^']*')+/g) ?? [], [command])
  const stages = useMemo<string[]>(() => ['TOKENIZE', 'PARSE', ...tokens.filter((token) => token !== '|' && !/^(<|>|>>)$/.test(token)).map((token, index) => `COMMAND ${index + 1} / ${token}`), ...(tokens.includes('|') ? ['PIPE'] : []), ...(tokens.includes('<') ? ['REDIRECT <'] : []), ...(tokens.includes('>') ? ['REDIRECT >'] : []), ...(tokens.includes('>>') ? ['REDIRECT >>'] : []), 'FORK', 'DUP2', 'EXECVE', 'WAITPID'], [tokens])
