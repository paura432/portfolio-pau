...
import { TrustModel } from '../../components/CasePlaygrounds'
...
export default function TrustosPage() {
  return <main className="case-page case-trustos"><CaseHeader chapter="03 / INFRASTRUCTURE" /><section className="trustos-hero"><p className="kicker">TELEFÓNICA TECH / 2025 — 2026</p><h1>TRUST<br/>OS</h1><p>Digital identity infrastructure for verifiable credential issuance and verification.</p><div className="credential-mark" aria-hidden="true">VC<br/>01</div></section><CaseDeepDive project="trustos" /><section className="case-split"><p className="case-label">APIs + PROVIDERS</p><div><h2>Identity is a system of trust.</h2><p>TrustOS VCS Core API provides a unified layer for credential lifecycle operations, provider integration, wallet flows and verification. Public detail stays at infrastructure level.</p></div></section><TrustModel /><section className="trustos-graph"><p className="case-label">CREDENTIAL FLOW</p><div><span>ISSUER</span><i>↓</i><span>VCS API</span><i>↓</i><span>PROVIDER ABSTRACTION</span><small>PRIVADOID · PROCIVIS · IDENFY</small><i>↓</i><span>WALLET</span><i>↓</i><span>VERIFY</span></div></section><footer className="case-footer"><Link href="/foundations">Next: Foundations <ArrowUpRight size={18}/></Link><Link href="/">All systems <ArrowUpRight size={18}/></Link></footer></main>
}
